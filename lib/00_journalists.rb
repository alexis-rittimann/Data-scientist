...
def small(mail)
    mail = mail.sort_by(&:length)
    puts mail[0]
end

def five(mail)
    five = mail.select {|handle| handle.tr('@','').length == 5}.size
    puts five
end

def upper(mail)
    first_upper = mail.select { |word| word.start_with?( /@[[:upper:]]/ ) }
    puts first_upper.length
end

def alphabétique(mail)
    ab = mail.sort_by{|handle| handle.downcase}
    puts ab
end

def small_to_big(mail)
    mail = mail.sort_by(&:length)
    puts mail
end

def wanted(mail)
    find = mail.index("@epenser")
    puts find
end

def nbr_handle_height(mail)
    taille = 0
    while taille == 25
    one = mail.select {|handle| handle.length == 1}.size
    puts one
    taille +=1
